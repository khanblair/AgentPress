...
function AgentStep({ agentKey, agentData, isLast }) {
      ...
      <div className="flex flex-col items-center">
        ...
        {!isLast && (
          <div className="w-px flex-1 mt-1"
            style={{ background: status === 'done' ? 'rgba(68,221,193,0.3)' : 'rgba(73,68,86,0.3)', minHeight: '2rem' }} />
        )}
      </div>
      ...
      <div className="flex flex-col gap-1 pb-6 flex-1 min-w-0">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold" style={{ color: status === 'pending' ? 'var(--outline)' : 'var(--on-surface)' }}>
              {agent.label}
            </span>
          ...
          {time && (
            <span className="text-xs shrink-0" style={{ color: 'var(--outline)' }}>{time}</span>
          )}
        </div>
      ...
      {job && (job.qa_retry_count > 0 || job.evolution_triggered) && (
        <div className="flex gap-2">
          {job.qa_retry_count > 0 && (
            <span className="text-xs px-2 py-1 rounded" style={{ background: 'rgba(251,191,36,0.1)', color: '#fbbf24', border: '1px solid rgba(251,191,36,0.2)' }}>
              QA retries: {job.qa_retry_count}
            </span>
          )}
          {job.evolution_triggered && (
            <span className="text-xs px-2 py-1 rounded" style={{ background: 'rgba(248,113,113,0.1)', color: '#f87171', border: '1px solid rgba(248,113,113,0.2)' }}>
              🧬 Evolution triggered
            </span>
          )}
        </div>
